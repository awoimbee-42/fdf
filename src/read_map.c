...
int				fill_map_line(char **data, t_map *map)
{
	...
	if (!(map->heightmap[map->size.y] = malloc((tmp - data + 1) * sizeof(int))))
		ft_msg_exit("cannot malloc heightmap line !", 0);
	data_len = 0;
	while (data[data_len])
	{
		height = ft_atoi(data[data_len]);
		if (height == 0 && *data[data_len] != '0')
			ft_msg_exit("Invalid file ! read : %s", data[data_len]);
...
static size_t	map_height(char *filename)
{
	...
	if (!(fd = open(filename, O_RDONLY)))
		ft_msg_exit("Cannot open file \"%s\"", filename);
	line_nb = 0;
	while (get_next_line(fd, &line) > 0 && ++line_nb)
		free(line);
	if (line_nb == 0)
		ft_msg_exit("Empty file ?", 0);
	if (close(fd) == -1)
		ft_msg_exit("Cannot close file", NULL);
	return (line_nb);
}
...
t_map			*actually_read(t_map *map, int fd)
{
	...
	if (i == -1)
		ft_msg_exit("Read error (malloc() or read() failed)", 0);
...
t_map			*read_map(t_map *map, char *filename)
{
	...
	if (!(map->heightmap = malloc(map->size.y * sizeof(int*))))
		ft_msg_exit("Not enought memory.", 0);
	if (!(fd = open(filename, O_RDONLY)))
		ft_msg_exit("Cannot open file \"%s\"", filename);
	map->size.y = 0;
