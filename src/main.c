...
void	init(t_data *dat, char *filename)
{
	...
	if (!(dat->zbuff = malloc((dat->map.size.y + 1) * sizeof(t_vertices*)))
		...
		ft_msg_exit("cannot allocate enough memory.", 0);
	i = -1;
...
void	usage(void)
{
	ft_msg_exit("Usage : ./fdf <filename> [-p] [-c] [-z] [-res]\n\
...
void	read_args(t_data *data, char **argv, int argc)
{
	...
	while (++i < argc)
	{
		if (ft_strcmp(argv[i], "-p") == 0)
			...
			ft_msg_exit("p cannot be equal to 0 !", 0) : 0;
			...
			ft_msg_exit("minimum zoom is 1", 0) : 0;
		else if (ft_strcmp(argv[i], "-res") == 0 && i + 2 < argc)
		{
			(data->win_width = ft_atoi(argv[++i])) < 10 ?
			ft_msg_exit("Window width too small ! (%dpx)", &data->win_width) : 0;
			(data->win_height = ft_atoi(argv[++i])) < 10 ?
			ft_msg_exit("Window height too small ! (%dpx)", &data->win_height) : 0;
		}
		else
			usage();
	}
...
int		main(int argc, char **argv)
{
	...
	if (!(data = ft_memalloc(sizeof(t_data))))
		ft_msg_exit("niet. malloc failed", 0);
	if (argc == 1 || argv[1][0] == '-')
		usage();
	init(data, argv[1]);
	...
	mlx_hook(data->mlx.win, 2, 1L, (void*)&keypress, data);
	mlx_loop_hook(data->mlx.ptr, (void*)&exec_render, data);
